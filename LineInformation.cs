...
namespace MongolNote
{
    ...
    public class CSLogLine//逻辑列，不皝E恍蟹模琇ines的元素，可皝E舾筛觯导氏允镜牧?
    {
        ...
        public static RealLine ArrangeALine(CSWord aWordLink,int LogLineIndex,int index,int StandardHeight)
        {
            ...
            while (WordPoint != null && tmpHeight + WordPoint.Height  <= StandardHeight )
            {
                tmp += WordPoint.Text;
        ...
        public void ArrangeLine(int MaxHeight, int LogLineIndex, int StandardHeight)
        {
            ...
            do
            {
                RealLine aRealLine = ArrangeALine(WordPoint,LogLineIndex, iii++,StandardHeight);
                LineList.Add(aRealLine);
